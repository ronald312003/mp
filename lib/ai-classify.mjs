...
/**
 * Utilidad genérica: prompt → JSON con gpt-5.4-mini (o null si falla).
 * Sirve de RESPALDO cuando Gemini se queda sin cuota (nivel gratuito).
 * Si el resultado natural sería una lista, el modelo la envuelve
 * como {"items":[...]} (exigencia del modo json_object).
 */
export async function openaiJSON(prompt, { tries = 2 } = {}) {
  const key = process.env.OPENAI_API_KEY;
  if (!key) return null;
  for (let i = 0; i < tries; i++) {
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                'Respondes SOLO JSON válido. Si el resultado pedido es una lista/array, devuélvelo envuelto como {"items":[...]}.'
            },
      ...
      if (i === tries - 1) console.warn(`  ! openai(${MODEL}): ${e.message}`);
